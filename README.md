...
Historical statistics on `/graph-earthquakes` come from a local **DuckDB** database, not from the USGS API. The USGS API has limitations that make it unsuitable for long-term statistical queries.
[DuckDB Quack](https://duckdb.org/docs/current/quack/overview) exposes the database as a separate service that `quakewatch` queries over the network — the same separation pattern used in Docker Compose and Kubernetes.

In Kubernetes, database seeding at startup is automated by `seed_data.py`. For building or debugging the database outside Compose/Kubernetes, see `NOTES.md`.
...
The Docker image is uploaded to the `mlsokolova` Docker Hub repository by the `main-merge.yaml` GitHub Actions workflow when a branch is merged into `main`.
The Docker image tag (version) comes from the branch name.
