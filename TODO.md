# TODO
## Technology
- Move the Parquet file containing the seed data from the GitHub repository to an AWS S3 bucket.

## Seismology (AI-generated section)
...
- [ ] **Regional comparison** — the same metrics for the selected area versus a country or continent baseline
...
- [ ] **Tsunamigenic events** — if the Parquet file includes tsunami flags, highlight offshore high-impact events (relevant for Japan, Chile, Indonesia, and the Eastern Mediterranean)
...
- Reuse the spatial filter pattern already used for `ST_Distance_Spheroid` around the selected latitude, longitude, and radius.
